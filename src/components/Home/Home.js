  ...
  const filteredData = (obj) => {
    ...
    if (obj.id === "CURRENT") {
      updatedfilteredData = (data || []).filter(
        (item) => getMonthDayYear(item.lastDate) === getMonthDayYear(new Date())
      );
    } else if (obj.id == "ALL") {
      updatedfilteredData = data;
    } else {
      updatedfilteredData = (data || []).filter(
        (item) => item.status == obj.id
      );
    }
    ...
    <div className="home">
      ...
      {!loading ? (
        <div className="h-container">
          {filterData?.length ? (
            filterData?.map((compliance, index) => {
              return (
            ...
            <div>
              {selectedFilter[0].id == "CURRENT" ? (
                <span>{`No Compliances for selected Month`}</span>
              ) : (
                <span>{`No ${selectedFilter[0].name} Compliances`}</span>
              )}
              <img src="/happy.png" />
            </div>
          )}
