...
const ManageUser = (props) => {
  ...
  const [statueList] = useState([
    { id: 1, title: "Companies Act" },
    { id: 2, title: "GST" },
    { id: 3, title: "Income Tax" },
  ]);
  ...
  const onDelete = (item) => {
    setSubmitLoading(true);
    AxiosInstance.delete(`${BASE_URL}${USER}${item.email}`)
      .then(() => {
  ...
  const renderRows = () => {
    return (
      <>
        {Array.isArray(props.userList?.data) &&
          props.userList?.data?.map((item, index) => {
            return (
              <tr>
                ...
                <td>
                  {" "}
                  <Button color="secondary" onClick={() => onEdit(item)}>
  ...
  return (
    <div className="user">
      <div className="header">
        ...
        <Button color="primary" className="button" onClick={openModal}>
          Add User
        </Button>
      </div>
      <CustomTable
